...
const PlayerOverview = ({ player, show, veto, players, round }: IProps) => {
    ...
    const getData = () => {
        ...
        const stats = veto.rounds.map(roundEntry => {
            if (!roundEntry) {
                return { kills: 0, killshs: 0, damage: 0 };
            }
            return roundEntry.players[player.steamid] || { kills: 0, killshs: 0, damage: 0 };
        });
        const overall = {
        ...
        const playedRounds = Math.max(1, round - 1);
        const data = {
            adr: stats.length !== 0 ? (overall.damage / playedRounds).toFixed(0) : '0',
            kills: overall.kills,
            killshs: overall.killshs,
