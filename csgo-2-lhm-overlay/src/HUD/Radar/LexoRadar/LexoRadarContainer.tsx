...
const LexoRadarContainer = ({ size = 300, mapName, bomb, player, players, grenades, reducedDetail = false }: IProps) => {
    ...
    const playersToRender = reducedDetail ? players.filter((pl) => pl.state.health > 0) : players;
    const grenadesToRender = reducedDetail
        ? grenades.filter((gr) => gr.type === "smoke" || gr.type === "inferno")
        : grenades;

    const playersExtended: RadarPlayerObject[] = playersToRender
        .map(pl => extendPlayer({ player: pl, steamId: player?.steamid || null, mapName }))
        .filter((player): player is RadarPlayerObject => player !== null)
        .flat();
    const grenadesExtended = grenadesToRender
        .map(grenade => extendGrenade({ grenade, side: playersExtended.find(player => player.steamid === grenade.owner)?.side || 'CT', mapName }))
